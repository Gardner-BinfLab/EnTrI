...
heatmappath <- "~/EnTrI/results/venn-entero-deg-endo/heatmap.tsv"
keggpath <- "~/EnTrI/results/venn-entero-deg-endo/heatmap-kegg.tsv"
...
while(ind <= nrow(heatmap))
{
  ...
  if(sum(row != prev_row))
  {
    ...
    if (nrow(pathways) > 0)
    {
      ...
      # print(pathways)
      pdf(paste("../figures/entero-deg-endo-",prev_row[1],prev_row[2],prev_row[3],prev_row[4],prev_row[5],".pdf",sep=''))
...
##################################################
# Comparing Enterobacteriaceae with DEG
##################################################

###### essential only in enterobacters
genes = kegg[heatmap$Enterobacteriaceae&!heatmap$Bacteria,1]
enrichment = data.frame(matrix(nrow=length(unique(db[,3])),ncol=4),row.names =unique(db[,3]) )
for (i in seq(1,nrow(enrichment)))
{
  for (j in seq(1,ncol(enrichment)))
