...
def read_cogs(filepath):
    cogdict = dict()
    with open(filepath, 'rU') as cogfile:
        line = cogfile.readline()
        for line in cogfile:
            cells = line.rstrip().split('\t')
            cogdict[cells[0]]=cells[1]
    return cogdict

def read_annotations(filepath, cogdict):
    annots = dict()
    with open(filepath, 'rU') as annotation_file:
        for line in annotation_file:
            cells = line.rstrip().split('\t')
            find_result = findall('COG(\d+)\@NOG', cells[8])
            if find_result:
                annots[cells[0]] = cogdict['COG' + find_result[0]]
            # annots[cells[0]] = cells[-1]
    return annots

seqdb = '../results/eggnog-mapper/seqdb.fasta.emapper.annotations'
coggroups = '../results/eggnog-mapper/COGs.tsv'
cogdict = read_cogs(coggroups)
seqdict = read_annotations(seqdb, cogdict)
