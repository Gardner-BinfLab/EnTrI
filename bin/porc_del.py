...
def define_cutoffs(lenfeatures, chunksize):
...
def calculate_distances(storepath, lenfeatures, chunksize, thresh, outpath):
    cutoffs = define_cutoffs(lenfeatures, chunksize)
    with pd.get_store(storepath) as store:
...
def find_centres(clusts, storepath, centrespath):
    with open(clusts) as clusterfile:
        with pd.get_store(storepath) as store:
            with pd.get_store(centrespath) as centres:
                for line in clusterfile:
                    genes = line.split()
                    lengenes = len(genes)
                    summation = store.select('features', where='index==genes').sum()
                    average = summation / lengenes
                    centres.append('centres', average)
...
def find_nearest_cluster(sequences, centrespath, storepath, seqs, lenfeatures, chunksize):
    # cutoffs = define_cutoffs(lenfeatures, chunksize)
    # with pd.get_store(storepath) as store:
    #     with pd.get_store(centrespath) as centres:
    #         for i, s in enumerate(cutoffs[:-1]):
    #             e = cutoffs[i + 1]
    #             chunk = centres.select('centres', start=s1, stop=e1)
    #             distances = cdist(chunk1, chunk2, 'euclidean')
    #             distances = list(chain.from_iterable(distances))



def pairwise_orthology_call(seq, outpath, clust, chunksize, thresh, evalue, temp, centres):
    ...
    if clust is None:
        ...
        find_nearest_cluster(sequences, clust, storepath, seqs, lenfeatures, chunksize)
    ...
    if centres:
        centrespath = outpath + '/centres.h5'
        find_centres(finalresult, storepath, centrespath)
...
def main():
    ...
    parser.add_argument('o', help='Path to the output', nargs='?', default=getcwd())
    parser.add_argument('-i', help='File containing initial centres')
    ...
    mkdir(temp)
    pairwise_orthology_call(seq, outpath, clust, chunksize, thresh, evalue, temp, centres)
    #rmtree(temp)
    stop = default_timer()
    print(stop - start)
