...
for (filename in list_of_files)
{
  gene_names = str_match(readLines(filename)[startsWith(readLines(filename),'>')],'>([[:graph:]]+)[[:blank:]]')[,2]
  genes <- c(genes, gene_names)
}

# allgenes <- str_match(readLines(seqdb)[startsWith(readLines(seqdb),'>')],'>([[:graph:]]+)[[:blank:]]')[,2]
